...
export class CustomersService {
  ...
  async create(accountId: string, dto: CreateCustomerDto) { return this.customerRepo.save(this.customerRepo.create({ ...dto, accountId })); }
  async findAll(accountId: string) { return this.customerRepo.find({ where: { accountId }, order: { createdAt: 'DESC' } }); }
...
export class OrdersService {
  ...
  async create(accountId: string, dto: CreateOrderDto) {
    ...
    try {
      ...
      return this.orderRepo.findOne({ where: { id: order.id }, relations: ['orderItems', 'invoice'] });
    } catch (err) { await qr.rollbackTransaction(); throw err; }
    finally { await qr.release(); }
  }

  async findAll(accountId: string) { return this.orderRepo.find({ where: { accountId }, relations: ['orderItems', 'customer', 'invoice'], order: { createdAt: 'DESC' } }); }

  async findOne(accountId: string, id: string) {
    const o = await this.orderRepo.findOne({ where: { id, accountId }, relations: ['orderItems', 'customer', 'invoice'] });
...
export class OrdersController {
  ...
  @Post() create(@CurrentAccount() user: any, @Body() dto: CreateOrderDto) { return this.ordersService.create(user.accountId, dto); }
  @Get() findAll(@CurrentAccount() user: any) { return this.ordersService.findAll(user.accountId); }
  @Get(':id') findOne(@CurrentAccount() user: any, @Param('id') id: string) { return this.ordersService.findOne(user.accountId, id); }
}
```
